...
ace.handle_side_menu = function(a) {
	...
	a(".nav-list").on( ace.click_event, function(h) {
		var target = (h.target.tagName == 'SPAN') ? h.target.parentElement : h.target;
		...
		var pageUrl = g.attr("href");
		if( pageUrl != "#" ){
			movePage(target.id, pageUrl+"?a="+parseInt(Math.random()*1000) );
			
			/* 메뉴 컨트롤 로직
				1. 전체 active 삭제
				2. 선택 아이템 부터 부모로 올라가며 active 추가
				3. open 된 것 중 active가 아닌 넘들 리셋
			--------------------------------------------------*/
			...
			// 2. 선택 아이템 부터 부모로 올라가며 active 추가
			var parentObject = g.parent();
			while( !parentObject.hasClass("nav-list") ){
			...
			// 3. open 된 것 중 active가 아닌 넘들 리셋
			$('.nav-list').find('.open').each( function() {
			...
			$('ul.breadcrumb').append(
				$('<li>').addClass('active').text(g.attr('title'))
			);
...
ace.general_things = function(a) {
	a('.ace-nav [class*="icon-animated-"]').closest("a").on(
			"click",
			function() {
				var d = a(this).find('[class*="icon-animated-"]')
						.eq(0);
				var c = d.attr("class").match(
						/icon\-animated\-([\d\w]+)/);
				d.removeClass(c[0]);
				a(this).off("click")
			});
	...
	a("#btn-scroll-up").on(
			ace.click_event,
			function() {
				var c = Math.min(400, Math.max(100, parseInt(a(
						"html").scrollTop() / 3)));
				a("html,body").animate({
					scrollTop : 0
				}, c);
	...
	a("#skin-colorpicker").on(
			...
			function() {
				...
				if (d == "skin-2") {
					a(".ace-nav > li").addClass(
							"no-border margin-1");
					a(".ace-nav > li:not(:last-child)").addClass(
							"light-pink").find(
							'> a > [class*="icon-"]').addClass(
							"pink").end().eq(0).find(".badge")
							.addClass("badge-warning")
				} else {
					a(".ace-nav > li").removeClass(
							"no-border margin-1");
					a(".ace-nav > li:not(:last-child)")
							.removeClass("light-pink").find(
									'> a > [class*="icon-"]')
							.removeClass("pink").end().eq(0).find(
									".badge").removeClass(
									"badge-warning")
				}
				if (d == "skin-3") {
					a(".ace-nav > li.grey").addClass("red").find(
							".badge").addClass("badge-yellow")
				} else {
					a(".ace-nav > li.grey").removeClass("red")
							.find(".badge").removeClass(
									"badge-yellow")
				}
			})
};
	...
	a(document)
			.on(
					...
					function(o) {
						...
						if (p == "collapse") {
							var j = b.hasClass("collapsed") ? "show"
									: "hide";
							var f = j == "show" ? "shown"
									: "hidden";
							var c;
							...
							if (p == "close") {
								var c;
								b.trigger(c = a
										.Event("close.ace.widget"));
								if (c.isDefaultPrevented()) {
									return
								}
								var r = parseInt(n
										.data("close-speed")) || 300;
								b
										.hide(
												r,
												function() {
...
ace.enable_search_ahead = function(a) {
	ace.variable_US_STATES = [ "Alabama", "Alaska", "Arizona",
...
ace.switch_direction = function(d) {
	...
	c
			.toggleClass("rtl")
			.find(
					".dropdown-menu:not(.datepicker-dropdown,.colorpicker)")
			...
			.find(".tmp-rtl-pull-right").removeClass(
					"tmp-rtl-pull-right").addClass("pull-left")
			.end().find(".chosen-container").toggleClass(
					"chosen-rtl").end();
	function a(h, g) {
		c.find("." + h).removeClass(h).addClass("tmp-rtl-" + h)
				.end().find("." + g).removeClass(g).addClass(h)
				.end().find(".tmp-rtl-" + h).removeClass(
						"tmp-rtl-" + h).addClass(g)
	}
	function b(h, g, i) {
		i.each(function() {
			var k = d(this);
